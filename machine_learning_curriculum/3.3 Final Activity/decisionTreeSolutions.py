...
def getPrediction(decisionTree, data):
    numCorrect = 0
    node = decisionTree
...
def getBestSplit(dataset):
    ...
    ############################################################################
    secondAttributeSplitOptions = [sortedDataset[i][1] for i in range(len(dataset)-1)]
...
def createDecisionTree(trainingData, maxDepth=50):
